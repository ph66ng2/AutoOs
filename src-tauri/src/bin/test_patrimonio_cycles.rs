...
async fn main() -> Result<()> {
    ...
    if !conflict.to_string().contains("AUTOOS_PATRIMONIO_SERIAL_CONFLICT") {
        anyhow::bail!("erro de conflito inesperado: {conflict:#}");
    }

    let empty_id = insert_equipment(&pool, &serial_other, Some("   ")).await?;
