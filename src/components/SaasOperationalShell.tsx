...
import { LockKeyhole, LogOut, Trash2, Users } from "lucide-react";
...
export function SaasOperationalShell({ session, onLock, onSignOut, onRemoveDevice }: SaasOperationalShellProps) {
  return (
    <BrowserRouter>
...
function SaasOperationalRoutes({ session, onLock, onSignOut, onRemoveDevice }: SaasOperationalShellProps) {
  return (
      ...
      <header className="border-b border-white/10 bg-slate-900/90">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-6 py-4">
          <div>
            ...
            <p className="text-sm text-slate-400">Plano Online · {session.identity.email}</p>
          </div>
